...
class Venue(db.Model):
    ...
    genres = db.Column("genres", db.ARRAY(db.String(300)), nullable=False)
...
# This is the time and date Format:
...
def venues():
  ...
  data = []
    # collect all the data for venues
  venues = Venue.query.all()
    
    # To make every data unique
  locations = set()
  ...
  for venue in venues:
    ...
    # Adding venues with state and city by avoiding duplications.
  ...
  for venue in venues:
    ...
    shows = Show.query.filter_by(venue_id=venue.id).all()
        # This uses the recent time to filter num_upcoming_shows
...
# function for searching for venues
...
def create_venue_submission():
  ...
  try:
    # Retrieve data from the venue form to create it.
  ...
  # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/
    ...
    # After the operation the session closes
    db.session.close()
...
def delete_venue(venue_id):
  ...
  try:
    ...
    # This deletes the selected venue
    db.session.delete(venue)
    db.session.commit()

    flash('Venue ' + venue_name + ' Successfully deleted')
  except:
    flash('Error!, unable to delete ' + venue_name)
    db.session.rollback()
  finally:
    db.session.close()
...
def show_artist(artist_id):
  ...
  # This filters the shows by previous shows and upcoming shows.
  for show in shows:
    data = {
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    db.session.rollback()
    flash('When attempting to upgrade Venue, an error occurred.')
  finally:
    db.session.close()
...
def create_artist_submission():
  ...
  try:
    ...
    db.session.commit()
    # on successful db insert, flash success
    flash('Artist ' + request.form['name'] + ' has been listed successfully!')
  except:
    print(sys.exc_info())
    db.session.rollback()
...
def delete_artist(artist_id):
  try:
    ...
    flash('Error!, Artist ' + artist_name + ' could not be deleted')
    db.session.rollback()
